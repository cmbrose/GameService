...
{
    $PublishProfileFile = "GameApplication\PublishProfiles\Local.1Node.xml"
}

if (!$ApplicationPackagePath)
{
    $ApplicationPackagePath = "GameApplication\pkg\Debug"
}
...
New-Item -ItemType Directory -Force -Path $ApplicationPackagePath
Copy-Item -Path "GameApplication\ApplicationPackageRoot\ApplicationManifest.xml" -Destination "$ApplicationPackagePath\ApplicationManifest.xml" -Force

foreach ($project in $ServiceFabricProjects)
{
    $PkgRoot = "$ApplicationPackagePath\" + $project + "Pkg\"
    dotnet publish $RootPath -o "..\$PkgRoot\Code" --no-build --configuration $Configuration
    Copy-Item -Path "$project\PackageRoot\*" -Recurse -Destination $PkgRoot -Force
}
